...
public class Metar {
  ...
  private String becoming = null;
	...
	protected void setPressure(Float value) {
          ...
          if( value != null )
	  {
            ...
            val = (float)Math.round(val*100)/100;
            this.pressure = new Float(val);
	  }
	  else {
	    this.pressure = value;
	  }
    ...
    public Integer getPressureInHectoPascals() {
      ...
      // convert to hPa
      float val = pressure.floatValue()*33.8639F;  
      // round
      return new Integer(Math.round(val)); 
    }
  ...
  /**
   * Get the "BECMG" string in raw form. We don't have the ability to parse
   * this string yet, so it's being stored in raw form for now.
   * @return Returns The becoming (BECMG) String in raw form.
   */
  public String getBecoming() {
    return becoming;
  }
  /**
   * Set the "BECMG" string in raw form. We don't have the ability to parse
   * this string yet, so it's being stored in raw form for now.
   * @param becoming The becoming (BECMG) String in raw form.
   */
  ...
  /**
   * Retrieve the full report string value. This is the report in its
   * original form
   * 
  ...
  /**
   * Set the full report string value. This is the report in its
   * original form
   * 
   * @param string  The original report string.
   */
  public void setReportString(String string) {
    reportString = string;
  }
